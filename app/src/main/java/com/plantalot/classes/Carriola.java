...
public class Carriola {
    ...
    public Carriola(Carriola other) {
        this.map = new HashMap<>(other.map.size());
        for (HashMap.Entry<String, HashMap<String, Integer>> entry : other.map.entrySet()) {
    ...
    public int countVarieta(String ortaggio) {
        return map.get(ortaggio) != null ? map.get(ortaggio).size() : 0;
    }
    ...
    public int countPiante(String ortaggio) {
        if (map.get(ortaggio) == null) return 0;
    ...
    public HashMap<String, Integer> getFamiglieCount() {
        ...
        for (String ortaggio : this.nomiOrtaggi()) {
            String varieta = this.nomiVarieta(ortaggio).iterator().next();
            Varieta varietaObj = this.getVarieta(ortaggio, varieta);
            if (varietaObj != null) {
                String famiglia = varietaObj.getTassonomia_famiglia();
                int count = this.countPiante(ortaggio);
                if (count > 0) {
                    famiglie.merge(famiglia, this.countPiante(ortaggio), Integer::sum);
                }
            }
        }
        return famiglie;
